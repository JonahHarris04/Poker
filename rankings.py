...
def has_flush(full_hand):
    ...
    if has_flush_bool:
        ...
        for number in full_hand_numbers:
            print(rank_number_to_string[number])
...
def has_two_pair(full_hand):
    ...
    pairs = []
    for card_number in full_hand_numbers:
        if full_hand_numbers.count(card_number) >= 2 and card_number not in pairs:
...
def has_high_card(full_hand):
    full_hand_numbers = [card.number for card in full_hand]
    if 1 in full_hand_numbers:  # Ace: maybe should emit 14? we'll see
        return True, 1
    return True, max(full_hand_numbers)


ranking = rank_hand(
    [Card.Card("Hearts", "A"), Card.Card("Spades", "3"), Card.Card("Clubs", "5"), Card.Card("Hearts", "4"),
     Card.Card("Hearts", "2"), Card.Card("Spades", "2"), Card.Card("Diamonds", "6")])
